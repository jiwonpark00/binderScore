...
1. Filter structures where min_distance <= per-target distance cutoff
...
The distance cutoff is specified per target in the residue CSV file
(columns: target_name, residue_number, distance_cutoff).
...
def extract_target_name(binder_name: str) -> str:
    ...
    # NOTE: keep in sync with calculate_distances.py and generate_boltz_yaml.py
...
def load_cutoff_map(residue_csv: Path) -> dict[str, float]:
    """
    Load per-target distance cutoff from residue CSV.
    
    CSV format: target_name,residue_number,distance_cutoff
    Returns dict like {"AQP": 20.0, "SLP": 15.0}
    
    Also stores lowercase keys for case-insensitive lookup.
    """
    cutoff_map = {}
    with open(residue_csv, 'r', newline='', encoding='utf-8-sig') as f:
        reader = csv.DictReader(f)
        
        # Validate that the distance_cutoff column exists
        fieldnames_lower = [fn.lower().strip() for fn in reader.fieldnames] if reader.fieldnames else []
        has_cutoff_col = any(fn in ('distance_cutoff', 'cutoff') for fn in fieldnames_lower)
        if not has_cutoff_col:
            raise ValueError(
                f"Residue CSV '{residue_csv}' is missing the 'distance_cutoff' column. "
                f"Expected columns: target_name, residue_number, distance_cutoff. "
        ...
        for row in reader:
            ...
            for key in row:
                ...
                elif key_lower in ('distance_cutoff', 'cutoff'):
                    try:
                        cutoff_val = float(row[key].strip())
                    except ValueError:
                        pass
            
            if target and cutoff_val is not None:
                cutoff_map[target] = cutoff_val
                cutoff_map[target.lower()] = cutoff_val
    
    return cutoff_map


def get_cutoff_for_binder(binder_name: str, cutoff_map: dict[str, float]) -> float:
    """
    Look up the distance cutoff for a binder by extracting its target name.
    
    Raises ValueError if no cutoff is found for the target.
    """
    target_name = extract_target_name(binder_name)
    cutoff = cutoff_map.get(target_name) or cutoff_map.get(target_name.lower())
    if cutoff is None:
        raise ValueError(
            f"No distance_cutoff found for target '{target_name}' (binder: '{binder_name}'). "
            f"Please add a distance_cutoff entry for '{target_name}' in the residue CSV."
        )
    return cutoff
...
def main():
    ...
    parser.add_argument("--residue_csv", required=True,
                        help="Residue CSV with target_name, residue_number, distance_cutoff columns")
    ...
    # Load per-target cutoff map from residue CSV
    cutoff_map = load_cutoff_map(residue_csv)
    print(f"Loaded per-target distance cutoffs from: {residue_csv}")
    for target, cutoff_val in sorted(set((k, v) for k, v in cutoff_map.items() if not k.islower() or k == k.lower())):
        # Print each unique target (avoid duplicates from case-insensitive keys)
        print(f"  {target}: {cutoff_val} Å")
    ...
    # Validate that all binders in distances CSV have a cutoff defined
    all_binders = set(row["binder"] for row in rows_attempt1 + rows_attempt2)
    missing_targets = []
    for binder in sorted(all_binders):
        try:
            get_cutoff_for_binder(binder, cutoff_map)
        except ValueError as e:
            missing_targets.append(str(e))
    
    if missing_targets:
        print(f"\nERROR: Missing distance_cutoff for {len(missing_targets)} binder(s):")
        for msg in missing_targets:
            print(f"  {msg}")
        print("\nPlease update the residue CSV with distance_cutoff values for all targets.")
        return 1
    ...
    for (binder, method), candidates_a1 in sorted(groups_attempt1.items()):
        ...
        # Look up per-target cutoff for this binder
        cutoff = get_cutoff_for_binder(binder, cutoff_map)
        ...
        if best is not None:
            ...
            if status == "passed":
                ...
                print(f"  {binder}/{method}: PASSED (dist={best['min_distance']}, cutoff={cutoff}, rank={best['model_rank']}, attempt={use_attempt})")
            else:
                method_stats[method]["failed"] += 1
                dist_str = f"{best['min_distance']}" if best.get("min_distance") else "N/A"
                print(f"  {binder}/{method}: FAILED (dist={dist_str}, cutoff={cutoff}, rank={best['model_rank']}, attempt={use_attempt})")
        else:
            summary_rows.append({
                "binder": binder,
                "method": method,
    ...
    fieldnames = [
        ...
        "num_candidates", "num_passing", "status", "attempt", "distance_cutoff"
    ]
    ...
    print("Summary Statistics")
    print(f"{'='*70}")
    print("Per-target distance cutoffs:")
    # Deduplicate: only show original-case keys
    seen_targets = set()
    for target, cutoff_val in sorted(cutoff_map.items()):
        if target not in seen_targets and target.lower() not in seen_targets:
            print(f"  {target}: {cutoff_val} Å")
